#!/usr/bin/env python3
"""Offline tests for the JCM33 dual-alignment 550 MHz BitcoinIII canary."""
...
RUNNER_PATH = ROOT / "run_jcm33_dualalign_btc3_550_canary.sh"
BUILD_PATH = ROOT / "build_jcm33_dualalign_bscan_550.tcl"
TOP_PATH = ROOT / "miner_top_ii1_bscan_550.sv"
...
class DualMinerTests(unittest.TestCase):
    ...
    def test_launcher_uses_packaged_550_test_filename(self):
        launcher = RUNNER_PATH.read_text()
        self.assertIn(
...
class DualAlignmentTests(unittest.TestCase):
    ...
    def test_build_is_timing_gated_and_uncompressed(self):
        ...
        self.assertIn("jcm33_dualalign_bscan_550.bit", build)
        self.assertIn("Performance_ExplorePostRoutePhysOpt", build)
        self.assertIn("-to_step write_bitstream", build)

    def test_candidate_clock_is_exactly_550_mhz(self):
        top = TOP_PATH.read_text()
        self.assertIn("module miner_top_ii1_bscan_550", top)
        self.assertIn(".CLKFBOUT_MULT_F(5.500)", top)
        self.assertIn(".CLKOUT0_DIVIDE_F(2.000)", top)
        self.assertIn("VCO = 1100 MHz", top)
    ...
    def test_runner_requires_both_devices_and_has_no_restore(self):
        ...
        self.assertIn("(( rejected == 0 ))", runner)
        self.assertIn("(( mismatches == 0 ))", runner)
        self.assertIn("build_jcm33_dualalign_bscan_550.tcl", runner)
