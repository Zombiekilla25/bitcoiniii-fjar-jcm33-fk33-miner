...
module fk33_bscan_transport (
    ...
    // A JCM33 has two six-bit TAPs in one physical JTAG chain.  The host
    // selects USER2 on one FPGA and BYPASS on the other, so every logical
    // byte arrives as a nine-bit DR word.  The TDO-near FPGA sees the bypass
    // bit before the payload; the TDI-near FPGA sees it after the payload.
    // Capture all nine bits and lock the byte slice from the leading FJ
    // magic.  This lets the same image work in either physical position.
    logic [8:0] rx_word2 = 9'h000;
    logic [3:0] rx_bit_count2 = 4'd0;
    logic rx_alignment_known2 = 1'b0;
    logic rx_alignment_high2 = 1'b0;
    ...
    wire [7:0] completed_rx_low2 = rx_word2[7:0];
    wire [7:0] completed_rx_high2 = {tdi2, rx_word2[7:1]};
    ...
    always_ff @(posedge drck2) begin
        if (capture2) begin
            ...
            rx_word2[rx_bit_count2] <= tdi2;
            if (rx_bit_count2 == 4'd8) begin
                if (rx_alignment_known2) begin
                    if (rx_alignment_high2)
                        consume_job_byte(completed_rx_high2);
                    else
                        consume_job_byte(completed_rx_low2);
                end else if (completed_rx_low2 == 8'h46) begin
                    rx_alignment_known2 <= 1'b1;
                    rx_alignment_high2 <= 1'b0;
                    consume_job_byte(completed_rx_low2);
                end else if (completed_rx_high2 == 8'h46) begin
                    rx_alignment_known2 <= 1'b1;
                    rx_alignment_high2 <= 1'b1;
                    consume_job_byte(completed_rx_high2);
                end
                rx_bit_count2 <= 4'd0;
            end else begin
