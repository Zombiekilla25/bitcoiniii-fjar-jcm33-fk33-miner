...
BUILD_TCL="$SCRIPT_DIR/build_jcm33_dualalign_bscan_525.tcl"
BUILD_LOG="$SCRIPT_DIR/build-jcm33-dualalign-525.log"
BITSTREAM="$SCRIPT_DIR/output/jcm33_dualalign_bscan_525.bit"
MINER="$SCRIPT_DIR/jcm33_btc3_dual_miner.py"
TESTS="$SCRIPT_DIR/test_jcm33_dualalign_btc3_canary.py"

EXPECTED_BRIDGE_SHA=fd1a550af5eb5dab475071a8f08f181c0b0d308233cbca805c52e3a96f342141

RUN_DIR="${TMPDIR:-/tmp}/jcm33-btc3-dual-canary-$(date -u +%Y%m%dT%H%M%SZ)-$$"
...
Usage:
  ./run_jcm33_dualalign_btc3_canary.sh --wallet bc1q... [options]

Options:
  --wallet ADDRESS       BitcoinIII bc1q address (required)
  --minutes N            Canary duration, 1-60 minutes (default: 10)
...
The canary builds a timing-gated 525 MHz production image whose USER2 parser
auto-detects both JCM33 chain positions, programs both FPGAs, and requires an
accepted pool share from A and B. It never changes voltage and contains no
...
build_image() {
    ...
    if [ -f "$BITSTREAM" ] &&
       ...
       [ "$REBUILD" -eq 0 ]; then
        ...
        echo
        echo "===== BUILDING DUAL-ALIGNMENT 525 MHz PRODUCTION IMAGE ====="
    ...
    while (( SECONDS < deadline )); do
        if [ -f "$log" ] &&
           ...
           grep -q 'SQRL JTAG Board 0 Device 1 Bitstream Loaded' "$log"; then
            echo "PASS: production 525 MHz image loaded on both JCM33 devices"
            return 0
        fi
        if ! kill -0 "$pid" 2>/dev/null; then
...
cleanup() {
    ...
    if [ "$IMAGE_PROGRAMMED" -eq 1 ]; then
        echo "INFO: dual-alignment 525 MHz production image left resident"
    else
        echo "INFO: resident FPGA images were unchanged"
    fi
    echo "INFO: no 350 MHz restore exists in this package"
    if [ "$status" -eq 0 ]; then
        echo "CANARY PASS: both independently aligned JCM33 FPGAs produced accepted BitcoinIII shares"
    else
...
echo "===== JCM33 DUAL-ALIGNMENT DUAL-FPGA BITCOINIII CANARY ====="
...
for command in python3 sha256sum tar ping pgrep ss grep timeout ldd stat cp tr; do
...
echo
echo "===== PROGRAMMING PRODUCTION 525 MHz IMAGE ====="
launch_program_bridge
