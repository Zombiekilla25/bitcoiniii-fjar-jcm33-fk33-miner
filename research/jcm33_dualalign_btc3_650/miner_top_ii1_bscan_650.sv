...
module miner_top_ii1_bscan_650 (
    input wire clk_p,
    input wire clk_n
);
    ...
    MMCME4_BASE #(
        ...
        .CLKIN1_PERIOD(5.000),
        .DIVCLK_DIVIDE(1),
        // 200 MHz * 6.500 / 2.000 = 650 MHz; VCO = 1175 MHz.
    ...
    BUFG u_bufg650(.I(clk650_mmcm), .O(clk650));

    (* ASYNC_REG="TRUE" *) logic [2:0] lock_sync650 = 3'b000;
    logic [3:0] release650 = 4'b0000;
    logic reset650 = 1'b1;

    always_ff @(posedge clk650) begin
        lock_sync650 <= {lock_sync650[1:0], mmcm_locked};
        if (!lock_sync650[2])
            release650 <= 4'b0000;
        else
            release650 <= {release650[2:0], 1'b1};
        reset650 <= ~release650[3];
    end
    ...
    // status. The mining engine runs at exactly 650 MHz.
    ...
    xpm_cdc_array_single #(
        ...
        .dest_clk(clk650), .dest_out(job_bus650)
    );
    ...
    xpm_cdc_single #(
        ...
        .dest_clk(clk650), .dest_out(job_toggle650)
    );

    wire [CAND_W-1:0] candidate_hold650;
    wire candidate_toggle650;
    wire [31:0] progress_gray650;

    sha3t_ii1_miner_engine u_engine (
        .clk(clk650),
        .reset(reset650),
        .job_toggle(job_toggle650),
        .job_bus(job_bus650),
        .candidate_hold(candidate_hold650),
        .candidate_toggle(candidate_toggle650),
        .progress_gray(progress_gray650)
    );
    ...
    xpm_cdc_array_single #(
        ...
        .src_clk(clk650), .src_in(candidate_hold650),
    ...
    xpm_cdc_single #(
        ...
        .src_clk(clk650), .src_in(candidate_toggle650),
    ...
    (* KEEP="TRUE" *) wire [31:0] progress_gray_debug = progress_gray650;
