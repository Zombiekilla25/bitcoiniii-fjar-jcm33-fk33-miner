...
module miner_top_ii1_bscan_500 (
    input wire clk_p,
    input wire clk_n
);
    ...
    MMCME4_BASE #(
        ...
        .CLKIN1_PERIOD(5.000),
        .DIVCLK_DIVIDE(1),
        // 200 MHz * 5 / 2 = 500 MHz; VCO = 1000 MHz.
    ...
    BUFG u_bufg500(.I(clk500_mmcm), .O(clk500));

    (* ASYNC_REG="TRUE" *) logic [2:0] lock_sync500 = 3'b000;
    logic [3:0] release500 = 4'b0000;
    logic reset500 = 1'b1;

    always_ff @(posedge clk500) begin
        lock_sync500 <= {lock_sync500[1:0], mmcm_locked};
        if (!lock_sync500[2])
            release500 <= 4'b0000;
        else
            release500 <= {release500[2:0], 1'b1};
        reset500 <= ~release500[3];
    end
    ...
    xpm_cdc_array_single #(
        ...
        .dest_clk(clk500), .dest_out(job_bus500)
    );
    ...
    xpm_cdc_single #(
        ...
        .dest_clk(clk500), .dest_out(job_toggle500)
    );

    wire [CAND_W-1:0] candidate_hold500;
    wire candidate_toggle500;
    wire [31:0] progress_gray500;

    sha3t_ii1_miner_engine u_engine (
        .clk(clk500),
        .reset(reset500),
        .job_toggle(job_toggle500),
        .job_bus(job_bus500),
        .candidate_hold(candidate_hold500),
        .candidate_toggle(candidate_toggle500),
        .progress_gray(progress_gray500)
    );
    ...
    xpm_cdc_array_single #(
        ...
        .src_clk(clk500), .src_in(candidate_hold500),
    ...
    xpm_cdc_single #(
        ...
        .src_clk(clk500), .src_in(candidate_toggle500),
    ...
    (* KEEP="TRUE" *) wire [31:0] progress_gray_debug = progress_gray500;
