...
Usage:
  ./start-btc3.sh [start] [options]
  ./start-btc3.sh doctor [options]  # read-only; does not program
  ./start-btc3.sh status
  ./start-btc3.sh logs [--lines N]
  ./start-btc3.sh stop [--force]
...
Start options:
  ...
  --canary-passed        Confirm a one-card BTC3 test produced matching
                         hardware/software hashes and an accepted pool share
...
show_plan() {
  local i port
  say "===== FK33 BitcoinIII FLEET PLAN ====="
  ...
  say "canary passed:  $CANARY_PASSED"
  say
  for i in "${!SERIALS[@]}"; do
    port=$((BASE_PORT + i))
...
start_fleet() {
  ...
  [[ "$CANARY_PASSED" == 1 ]] ||
    die "fleet start requires --canary-passed after a matching hw/sw digest and accepted BTC3 share"
  ...
  already_running && die "fleet bridge is already running; use './start-btc3.sh status'"
  ...
  STARTED_WORKER_PIDS=()
  say
  say "PASS: BTC3 fleet started."
  say "Status: ./start-btc3.sh status"
  say "Logs:   ./start-btc3.sh logs"
  say "Verify every worker remains authorized and produces accepted shares."
}
...
stop_pid() {
  ...
  if (( FORCE_STOP == 1 )); then
    ...
    warn "$label is still running; rerun './start-btc3.sh stop --force' only after checking it"
    return 1
  fi
}
