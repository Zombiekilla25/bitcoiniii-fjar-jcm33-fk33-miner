#!/usr/bin/env bash
# FK33 BitcoinIII one-card canary launcher
#
# This launcher never changes voltage and refuses to start more than one card.
# The SHA3-256T 525 MHz image is fleet-tested on FK33/FJAR, but BTC3 deployment
# remains a canary until a hardware/software digest match and accepted pool
# share are observed.

set -Eeuo pipefail
IFS=$'\n\t'

readonly APP_NAME="fk33-btc3-canary"
...
BASE_PORT="${BTC3_BASE_PORT:-$DEFAULT_BASE_PORT}"
WORKER_PREFIX="${BTC3_WORKER_PREFIX:-$(hostname -s 2>/dev/null || printf 'fk33')}"
...
Usage:
  ./start-btc3-canary.sh [start] [options]
  ./start-btc3-canary.sh doctor [options]  # read-only; does not program
  ./start-btc3-canary.sh status
  ./start-btc3-canary.sh logs [--lines N]
  ./start-btc3-canary.sh stop [--force]

Start options:
  --wallet ADDRESS       Your BitcoinIII/BTC3 bc1q address (required)
  --serials SERIAL       Exactly one FK33 USB serial (required for canary)
...
Optional config-btc3.env entries:
  ...
  BTC3_POOL_PORT, BTC3_BASE_PORT

Package layout:
  start-btc3-canary.sh
...
load_config() {
  ...
  BASE_PORT="${BTC3_BASE_PORT:-$BASE_PORT}"
  WORKER_PREFIX="${BTC3_WORKER_PREFIX:-$WORKER_PREFIX}"
...
validate_wallet() {
  [[ "$WALLET" =~ ^bc1q[023456789acdefghjklmnpqrstuvwxyz]{20,86}$ ]] ||
    die "missing or invalid BTC3 wallet; use --wallet 'bc1qYOUR_ADDRESS'"
}
...
validate_files() {
  ...
  [[ "$actual_sha" == "$STABLE_BIT_SHA256" ]] ||
    die "BTC3 canary requires the pinned 525 MHz SHA3-256T image; got $actual_sha"
  BITSTREAM_MODE="pinned 525 MHz SHA3-256T image (BTC3 canary)"
...
resolve_serials() {
  ...
  (( ${#SERIALS[@]} == 1 )) ||
    die "BTC3 canary requires exactly one --serials value; fleet start is intentionally blocked"
}
...
show_plan() {
  local i port
  say "===== FK33 BitcoinIII ONE-CARD CANARY PLAN ====="
  ...
  say "gate:           digest match + accepted BTC3 share required"
  say
  for i in "${!SERIALS[@]}"; do
    port=$((BASE_PORT + i))
...
start_fleet() {
  ...
  already_running && die "canary bridge is already running; use './start-btc3-canary.sh status'"
  ...
  STARTED_WORKER_PIDS=()
  say
  say "PASS: BTC3 one-card canary started."
  say "Status: ./start-btc3-canary.sh status"
  say "Logs:   ./start-btc3-canary.sh logs"
  say "Do not scale to the fleet until a [SHARE] hw=.../sw=... match and [ACCEPTED] line appear."
}
...
stop_pid() {
  ...
  if (( FORCE_STOP == 1 )); then
    ...
    warn "$label is still running; rerun './start-btc3-canary.sh stop --force' only after checking it"
    return 1
  fi
}
...
stop_fleet() {
  ...
  pid="$(read_pid "$STATE_DIR/pids/bridge.pid")"
  if [[ -n "$pid" ]] && ! stop_pid "$pid" "$BRIDGE" "canary bridge"; then
    failed=1
  fi

  if (( failed == 0 )); then
    find "$STATE_DIR/pids" -maxdepth 1 -type f -name '*.pid' -delete 2>/dev/null || true
    say "PASS: FK33 BitcoinIII processes stopped. FPGA voltage was not changed."
  else
